...
/**
 * The EncryptorEngine class contains the core business logic for the
 * application.
...
public class EncryptorEngine {
    ...
    /**
     * Encrypts a file using the given password and deletes the original file upon
     * success.
     ...
     * @throws IOException If file reading/writing fails or if the original file
     *                     cannot be deleted.
     */
    ...
    /**
     * Decrypts a ".enc" file using the given password and deletes the encrypted
     * file upon success.
     ...
     * @throws IOException If file reading/writing fails, the password is wrong, or
     *                     deletion fails.
     */
    public static void decrypt(String filePath, String password) throws IOException {
        ...
        // Ensure the user is trying to decrypt a file that actually has the .enc
        // extension
    ...
    /**
     * A private helper method that performs the actual reading, XOR transformation,
     * and writing.
     ...
     * @throws IOException If an error occurs during file I/O or if the password
     *                     verification fails.
     */
    private static void processFile(File inputFile, File outputFile, String password, boolean isEncrypt)
            throws IOException {
        ...
        try (FileInputStream fis = new FileInputStream(inputFile)) {
            ...
            // If we are decrypting, we must first verify the password before creating an
            // output file
            if (!isEncrypt) {
                ...
                int storedHash = ((lenBytes[0] & 0xFF) << 24) |
                        ((lenBytes[1] & 0xFF) << 16) |
                        ((lenBytes[2] & 0xFF) << 8) |
                        (lenBytes[3] & 0xFF);
                ...
                // Check if the stored password hash matches the provided password hash
                if (storedHash != hash) {
                    throw new IOException("Wrong password! Cannot decrypt file.");
                }
            }
            ...
            try (FileOutputStream fos = new FileOutputStream(outputFile)) {
                ...
                // If we are encrypting, we need to write the verification header to the new
                // file
                if (isEncrypt) {
                    ...
                    // Write the hash as 4 separate bytes using bitwise shifts
                    fos.write(new byte[] {
                            (byte) (hash >>> 24),
                            (byte) (hash >>> 16),
                            (byte) (hash >>> 8),
                ...
                while ((bytesRead = fis.read(buffer)) != -1) {
                    ...
                    // Keep track of how many bytes we've processed so the password index stays
                    // correct
